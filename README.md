...
2. **Presence-confirmed whisper sync**: when a player joins GreenWall's normal guild bridge channel, GuildRoster waits briefly and sends a small handshake. Only a client that answers that handshake is eligible for one full roster exchange. The receiving client acknowledges only one sender per co-guild during a cooldown, so a single channel join cannot make every visible addon client send the same roster. The subsequent full payload is throttled and chunked. `/who` is not used at all: it neither discovers targets nor creates roster rows.
